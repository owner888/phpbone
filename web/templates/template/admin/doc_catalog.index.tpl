...
{
    tb_show('增加分类', '?ct=doc_catalog&cmid=<{$infos.cmid}>&even=add&TB_iframe=true&height=350&width=600', true);
}
function show_data(nid)
{
    tb_show('浏览/编辑分类', '?ct=doc_catalog&cmid=<{$infos.cmid}>&even=edit&cid='+ nid +'&TB_iframe=true&height=350&width=600', true);
}
...
$(function(){
    ...
    $('.expand').click(function(){
        ...
        if( curimg.attr('src') != '../../static/frame/admin/images/m-close.gif' )
        {
            ...
            tb_show_loading();
            $.get("?ct=doc_catalog&ac=ajax_load_son&cmid="+cmid+"&cid="+cid, function(data){
                $('#son_'+cid).html( data );
                curimg.attr('src', '../../static/frame/admin/images/m-last.gif');
                tb_remove();
            });
